...
from app.keyword import schemas
from app.keyword.service import KeywordService as keyword_service
...
async def read_users(db: Session = Depends(get_db)):
    keywords = keyword_service.get_all_keywords(db)
    return keywords
...
async def read_user(keyword_id: str, db: Session = Depends(get_db)):
    db_keyword = keyword_service.get_keyword(db, keyword_id=keyword_id)
